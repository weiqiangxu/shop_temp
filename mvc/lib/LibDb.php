...
class LibDb
{
    ...
    public function Query($sql)
    {
		...
		{
			case 'pgsql': $sql = str_replace('\\\'', "''", $sql); break;
		}
        $this->sql = $sql;
		try {
    ...
    */
	protected function Connect($IsReConnect = false)
    {
        ...
        switch($this->configs['db_type'])
        {
            ...
            case 'mysql':
                ...
                break;
			case 'oci':
				$sid = empty($this->configs['db_sid']) ? 'yiparts': $this->configs['db_sid'];
                $this->dsn = "oci:dbname=(DESCRIPTION=(ADDRESS=(PROTOCOL=TCP)(HOST=".$this->configs['db_host'].")(PORT=".$this->configs['db_port']."))(CONNECT_DATA=(SID=".$sid.")))";
                try{
					if($this->configs['db_charset']<>''){
                ...
                break;
			case 'pgsql':
                $this->dsn = "pgsql:dbname=".$this->configs['db_name'].";host=".$this->configs['db_host'].";port=".$this->configs['db_port'];
                try{
					$this->pdo = new PDO($this->dsn, $this->configs['db_user'], $this->configs['db_pass']);
                } catch (PDOException $e) {
                    $this->ToLog($e->getMessage());
    ...
    {
		$message = "{$message}\r\n[SQL]{$this->sql}\r\n";
		$logFile = dirname ( __FILE__ ).'/dblog/';
		@mkdir($logFile);
