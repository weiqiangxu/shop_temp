...
class LibFc
{
	...
	/**
	  ...
	  * @return [["status"]=> bool(true) ["data"]=> [1=>['PART_ID'=>1, 'PART_NAME'=>''], ....] ["code"]=> int(0) ]
	  */
	static function formatFac($string)
	{
	...
	/**
	  ...
	  * @return [["status"]=> bool(true) ["data"]=> [1=>['PART_ID'=>1, 'PART_NAME'=>''], ....] ["code"]=> int(0) ]
	  */
    static function FormatVin($Vin)
    {
        $Vin = strtoupper(str_replace(' ', '', $Vin));
		$ReArr = ['０' => '0', '１' => '1', '２' => '2', '３' => '3', '４' => '4','５' => '5', '６' => '6', '７' => '7', '８' => '8', '９' => '9',
        'Ａ' => 'A', 'Ｂ' => 'B', 'Ｃ' => 'C', 'Ｄ' => 'D', 'Ｅ' => 'E','Ｆ' => 'F', 'Ｇ' => 'G', 'Ｈ' => 'H', 'Ｉ' => '1', 'Ｊ' => 'J', 'Ｋ' => 'K',
        'Ｌ' => 'L', 'Ｍ' => 'M', 'Ｎ' => 'N', 'Ｏ' => '0','Ｐ' => 'P', 'Ｑ' => '0', 'Ｒ' => 'R', 'Ｓ' => 'S', 'Ｔ' => 'T','Ｕ' => 'U', 'Ｖ' => 'V',
        'Ｗ' => 'W', 'Ｘ' => 'X', 'Ｙ' => 'Y','Ｚ' => 'Z', 'ａ' => 'A', 'ｂ' => 'B', 'ｃ' => 'C', 'ｄ' => 'D','ｅ' => 'E', 'ｆ' => 'F', 'ｇ' => 'G',
        'ｈ' => 'H', 'ｉ' => '1','ｊ' => 'J', 'ｋ' => 'K', 'ｌ' => 'L', 'ｍ' => 'M', 'ｎ' => 'N','ｏ' => '0', 'ｐ' => 'P', 'ｑ' => '0', 'ｒ' => 'R',
        'ｓ' => 'S', 'ｔ' => 'T', 'ｕ' => 'U', 'ｖ' => 'V', 'ｗ' => 'W', 'ｘ' => 'X', 'ｙ' => 'Y', 'ｚ' => 'Z', 'I'=>'1', 'O'=>'0', 'Q'=>'0'];
        $Vin = trim(strtr($Vin, $ReArr));
        return preg_replace('/[^0-9a-zA-Z]/', '', $Vin);
    }
	...
	/**
	  ...
	  */
	static function keywordRed($string, $keyword)
	{
		if(strlen($keyword) == 0 || strlen($string) == 0) return $string;
		$ignor = [
		'~',  '!',  '@',  '#',  '$',  '%',  '^',  '&',  '*',  '(',  ')',  '-',  '+',  '[',  ']',
		':',  ';',  '\'', '"',  '|',  '\\', ',',  '.',  '?',  '/',  '<',  '>',
		];
		$keyword = str_ireplace($ignor,' ',$keyword);
		$keyword = preg_replace('/\s+/i','|',$keyword);
		return preg_replace("/($keyword)/i",'<font color="red">\\1</font>',$string);
	}
     ...
     */
	static function formatViewYear($startYear, $endYear, $type=1, $style= null)
	{
		$out = '';
		switch($type)
		{
			...
			case 2:
				...
				if(!empty($startYear) || !empty($endYear))
				{
					$out = empty($endYear)? $startYear.'-至今': $startYear.'-'.$endYear;
				}
			break;
		}
		$out = $out != '' && $style == 1? '【'.$out.'】': $out;
		return $out;
	}
	...
	static function fenCi($word, $dictFile = [])
	{
        ...
        $so->set_charset('utf8');
		//设定分词返回结果时是否去除一些特殊的标点符号之类 默认为false
        $so->set_ignore(true);
		$so->set_multi(true);
        $so->send_text($word);
	 ...
	 * @copyright soul 2017/9/20
     */
	static function fromatKeyword($word)
	{
		$arr = array('&nbsp;', ' ', ',', '，', '。', '、', '/', '|', ';', '；', '\\');
        $wordArr = explode(',', str_ireplace($arr, ',', $word));
		$wordArr = array_unique($wordArr);
		foreach($wordArr as $k=>$v)
		{
			if($v == '') unset($wordArr[$k]);
		}
	...
	static function RegularValida($str,$type)
	{
		...
		switch($type)
		{
			...
			// 密码
			case 'password':
				$data = preg_match("/^\S{6,12}$/u",$str);//6到12个任意非空白字符
			break;
			// 邮箱
			case 'email':
				$data = preg_match("/^\w+((-\w+)|(\.\w+))*\@[A-Za-z0-9]+((\.|-)[A-Za-z0-9]+)*\.[A-Za-z0-9]+$/",$str);//6到12个任意非空白字符
			break;
			// 手机号码
			case 'mobile':
				// $data = preg_match("/^\S{6,12}$/u",$str);//6到12个任意非空白字符
     ...
     * 
     * @method                返回图片路由
	 * @param [string|data]    $data  要求输入字段
	 * @param [string]        $type 字段名称
	 * @copyright             xu   2018/04/13
	 * @return                array
     */
	static function ImgRoute($data,$type)
	{
		...
		switch($type)
		{
			// 营业执照
			case 'license': 
				$str = mvc::$cfg['FILE_CENTER']['b2c']['url_get'].'&c=ufile&f='.$data.mvc::$cfg['FILE_CENTER']['b2c']['get_uri'];
				break;
			// 身份证
			case 'card':
				if(!empty($data[1]) && $data[1] == 100){
					$str = mvc::$cfg['FILE_CENTER']['b2c']['url_get'].'&c=ufile&f='.str_replace('100/','',current($data)).mvc::$cfg['FILE_CENTER']['b2c']['get_uri'];
				}else{
					$str = mvc::$cfg['FILE_CENTER']['b2c']['url_get'].'&c=ufile&f='.current($data).mvc::$cfg['FILE_CENTER']['b2c']['get_uri'];;
				}
			break;
			// 店铺logo;data = ['logo','size']
			case 'logo':
				if(!empty($data[1]) && $data[1] == 100){
					$str = mvc::$cfg['FILE_CENTER']['b2c']['url_get'].'&c=shopLogo&f='.str_replace('100/','',current($data)).mvc::$cfg['FILE_CENTER']['b2c']['get_uri'];
				}else{
					$str = mvc::$cfg['FILE_CENTER']['b2c']['url_get'].'&c=shopLogo&f='.current($data).mvc::$cfg['FILE_CENTER']['b2c']['get_uri'];
				}
			break;
			case 'pro':
				// 商品;data = [path,name,size]
				if($data[2]==100){
					$str = mvc::$cfg['FILE_CENTER']['b2c']['url_get'].'&c=product&f='.$data[0].'/100/'.$data[1].mvc::$cfg['FILE_CENTER']['b2c']['get_uri'];
				}else{
					$str = mvc::$cfg['FILE_CENTER']['b2c']['url_get'].'&c=product&f='.$data[0].'/'.$data[1].mvc::$cfg['FILE_CENTER']['b2c']['get_uri'];
				}
		}
		return $str;
	}
	 ...
	 * @return                array
     */
	static function formatFustr($str) {
		...
		$str = mb_substr($str, 0, 300,"utf-8");//返回字符串中的前100字符串长度的字符
		return $str;
	}
